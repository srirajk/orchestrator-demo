...
## Key Learnings (S4/S5 break-glass runtime-load flake, 2026-07-17)
- Cerbos `watchForChanges` (disk driver) processes each policy file's storage event INCREMENTALLY and resolves a scoped policy's ancestor from the CURRENT in-memory index — NOT by re-scanning the dir. If a scoped child (scope "acme") is indexed before its same-version root (scope ""), it fails `failed to load ancestor ... policy not found` and Cerbos "maintains last valid state" and NEVER retries that child. So a self-contained bundle (whole scope chain stamped to one bundleId) MUST be written ANCESTOR-FIRST.
- `PromotedBundleLoader` wrote `renderedFiles()` in PATH order → `agent@acme.yaml` ('@'=0x40) sorts before `agent_resource.yaml` ('_'=0x5f) → child-before-ancestor → deterministic un-enforced promotion. Fix: write by scope-depth tiers (root=0, acme=1) with a settle between tiers.
- Docker Desktop macOS VirtioFS bind-mount inotify is HIGH-LATENCY/lossy: host→container file events arrive seconds late or coalesce. Sub-second inter-tier settle dropped ~50% of reloads; a 3s settle (config `iam.policy-studio.runtime-load-tier-settle-ms`, default 3000) reliably lets each ancestor tier propagate+index before children land. Promotion is control-plane/off-request-path so the latency is acceptable; configurable down on fast native-inotify hosts.
- Debugging watch-reload flakes: reproduce by running the pinned Cerbos server on the loader's temp dir (temp dirs from `Files.createTempDirectory` are NOT cleaned up — find them under $TMPDIR) and replaying CheckResources; a STATIC load (all files present at startup scan) always resolves correctly, which isolates "policy correct, delivery flaky" from a real policy bug.
- Spring: a @Component with TWO constructors needs exactly one annotated `@Autowired` or context creation is ambiguous — annotate the full @Value constructor; leave the convenience one plain.
