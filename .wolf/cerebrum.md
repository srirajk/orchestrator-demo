...
## Key Learnings (Cerbos scope-posture, added 2026-07-16 — Axiom B1, EMPIRICALLY proven vs 0.53.0)
- `ghcr.io/cerbos/cerbos:latest` present locally IS version **0.53.0** (build commit 78d494cab, digest sha256:c3fe64202e37). The `:0.53.0` tag pull is network-blocked offline; use `latest` (same binary) and record the delta.
- **Cerbos scope semantics (the load-bearing fact):** under `SCOPE_PERMISSIONS_REQUIRE_PARENTAL_CONSENT_FOR_ALLOWS`, a child (tenant) scope that is SILENT on a base-allowed (resource,action,role) tuple does NOT deny it — it INHERITS the parent ALLOW (fail-open). Parental consent only constrains tuples the child EXPLICITLY allows (they must also be allowed by the parent). ⇒ an empty/stub tenant policy is fail-open; "ALLOW with a false condition = implicit deny" is ALSO fail-open (non-match = silence = inherit parent). The ONLY safe deny-by-default is an explicit `EFFECT_DENY` per withheld ceiling tuple (DENY beats ALLOW; child DENY overrides parent ceiling).
- The **base/root scope must use `OVERRIDE_PARENT`, NOT parental-consent** — a root that requires parental consent denies everything (no parent can consent). REQUIRE_PARENTAL_CONSENT goes on the TENANT scopes.
- `engine.lenientScopeSearch: false` (strict, explicit) makes a request for an ABSENT tenant scope hard-DENY. With `true` it walks up to the base ceiling → an unregistered tenant inherits everything (fail-open). Pin it explicitly.
- Policy validation + `_test.yaml` execution is ONE command: `cerbos compile --test-output=junit <dir>` (there is NO standalone `cerbos test`). `--test-output=junit` and config keys `engine.lenientScopeSearch` + `audit.decisionLogsEnabled` all confirmed valid on 0.53.0.
- Proof harness runs on DISTINCT ports/names (3600/3601, 3610/3611; cerbos-proof-b1) — never disturb the demo's conduit-cerbos (:3594/:3595) or the unrelated backend-cerbos-1 (:3592, cerbos 0.40.0).
