...
- ArchUnit 1.3.0 (its shaded ASM) CANNOT read Java 25 bytecode (major 69) — throws "Unsupported class file major version 69" and rules see 0 classes → failOnEmptyShould failure. The gateway avoids this by compiling to bytecode target 21; **iam-service compiles to target 25**, so its ArchUnit tests need **archunit-junit5 1.4.1** (bundles ASM 9.8). Use 1.4.1 for any iam-service ArchUnit test.
- Cerbos scopes are dot-delimited hierarchical; compare containment by PARSED SEGMENTS, never string prefix (`tenant.a` must not match `tenant.ab`). Mirrors scripts/cerbos-tenant-totality-lint.py scope_ancestors().
- cerbos-allow-tenant-equality-lint.py EXEMPTS tenant restriction children (REQUIRE_PARENTAL_CONSENT_FOR_ALLOWS) from the raw-role-ALLOW rule — the base ceiling carries the backstop; a generated child (like tenant_default_agent.yaml) can use raw roles.
