...
| --:-- | Axiom A1 tenant model + token contract (branch feature/axiom-A1-token-contract) | iam-service AuthController/JwtClaimsCustomizer/OidcClaimEnricher + TenantClaims/ServiceTenantProperties/DelegationRequest; gateway SecurityConfig/Principal; BFF+gateway tests | tenant_id mandatory+non-defaultable on all mint paths; tenant-qualified aud conduit-gateway@<tenant>; typed delegation token rejected by data path; X-User-Id fenced; world-b 0→0; IAM 8 tests green | ~large |
