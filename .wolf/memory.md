...
| C3 | Axiom Story C3 — independent test-scenario generation (the oracle-problem fix / moat). New policystudio types: TestScenarioRequest (NO YAML field — type-level fence), TestScenarioModelClient (oracle seam, diff prompt from C2 generator), Expectation/Effect/ProbeKind, TestExpectationSet(renders _test.yaml), ProbeAttributes, NegativeProbeInjector (3 probes/allow), ConditionEvaluator (tight CEL subset, fail-closed missing-attr), PolicyExpectationEvaluator (candidate decision w/ fall-through+backstop), IndependentTestGenService+CatchLayer (draft gate = oracle+probes+C2 validator+optional Cerbos compile=B3 invariants). Harness (seeded oracle, NO LLM): C3Corpus 31 known-bad pairs/7 classes, KnownBadCorpusCatchRateTest (MEASURED 100% 31/31, 100% cross-tenant+wildcard, 13 moat-critical, 0 misses), TestGenIsolationArchTest (reflection+ArchUnit: no YAML in oracle input), NegativeProbeInjectionTest. | iam-service/.../policystudio/* (+11 main, +4 test), docs/implementation/evidence/studio/c3/* | iam mvn -o test 30/30 (was 22); world-b CRITICAL 0→0; cerbos-policy-gate 74/74 (running conduit-cerbos untouched, ephemeral) | ~140k |
