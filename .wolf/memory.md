...
| 04:10 | Axiom B2: migrate 6 Cerbos policies to base/tenant scoped model — business_derived_roles (tenant-equality backstop), base scope on 5 resources, relationship gap closed, default-tenant TOTAL children, domain→segment mapping relocated to tenants/default manifest; harness: parity-matrix(800 cells PRE=POST), 5 named cerbos suites (45 OK), tenant-equality lint + totality lint (derived-role expanded) | infra/cerbos/policies/*, infra/cerbos/tenants/default/, scripts/cerbos-*.py|.sh | PARITY byte-identical; world-b CRITICAL 0; all gates green | ~60k |
