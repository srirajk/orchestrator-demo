...
| 22:32 | Axiom B1 scope-posture: PROVEN vs cerbos 0.53.0 (latest==0.53.0, digest c3fe64202e37). Contract#2 CONFIRMED — silent tenant tuple INHERITS parent ALLOW (empty tenant policy=fail-open); strict lenientScopeSearch:false makes absent tenant hard-DENY (lenient=fail-open). Base=OVERRIDE_PARENT ceiling, tenant=REQUIRE_PARENTAL_CONSENT grants; ceiling-exceed denied. Deny-all template uses explicit EFFECT_DENY (false-condition ALLOW is ALSO fail-open). 11/11 junit tests pass; totality lint catches holes. | infra/cerbos/{config.yaml,proof,templates}, scripts/cerbos-tenant-totality-lint.py, docs/adr/ADR-axiom-scope-posture.md | proof green, world-b CRITICAL 0, request path untouched | ~B1 |
