...
| C2 | Axiom Story C2 NL→YAML policy studio: PolicyAuthoringModelClient (authoring-only) + com.openwolf.iam.policystudio deterministic gate (parser rejects aliases/tags/non-resource-policy, validator: scope-subtree segment-wise + vocab grounding + finiteness + backstop + totality + identity, canonical writer, cerbos compile gate) + 6 named tests + ArchUnit boundary | iam-service/src/{main,test}/java/com/openwolf/iam/policystudio, iam-service/pom.xml | 22/22 iam tests green (JDK25), world-b CRITICAL 0, cerbos gate 74/74 | ~40k |
