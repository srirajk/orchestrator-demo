...
If the user asks for a figure that is not in the list — a total, average, percentage, allocation, or any derived or converted value — do NOT calculate or estimate it. Say that figure is not available in the retrieved data and present the listed figures individually. Never volunteer a derived statistic either: any value that would require arithmetic on the data — a ratio, reciprocal, rescaling, index transform, or an "effective number" — may appear only if that exact value is already present verbatim in the DATA or GROUNDED FIGURES; otherwise omit it entirely. Counting items actually listed in the DATA (e.g. "two accounts") is still allowed.
