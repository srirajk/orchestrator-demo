"""
OTel + OpenInference span helper for the HR Policy HTTP service.
...
SERVICE_NAME = os.getenv("OTEL_SERVICE_NAME", "conduit-hr-policy")
...
def agent_span(agent_id: str, input_value: str = None):
    ...
    Usage:
        with agent_span("meridian.hr.policy_qa", input_value=topic) as span:
            span.set_attribute("result.topic", topic)
    """
    ...
    def _span():
        with tracer.start_as_current_span(f"agent.{agent_id.split('.')[-1]}") as span:
            ...
            span.set_attribute("agent.domain", "hr")
            span.set_attribute("agent.audience", "enterprise")
